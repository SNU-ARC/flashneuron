...
class DeviceCachingAllocator {
 ...
 public:
  ...
  Block* malloc(int device, size_t size, cudaStream_t stream, bool reverse)
  {
...
class THCCachingAllocator {
 ...
 public:
  ...
  void malloc(void** devPtr, int device, size_t size, cudaStream_t stream, bool reverse) {
    TORCH_INTERNAL_ASSERT(
        0 <= device && device < device_allocator.size(),
        "Allocator not initialized for device ",
        device,
        ": did you call init?");
    Block* block = device_allocator[device]->malloc(device, size, stream, reverse);
...
struct CudaCachingAllocator : public Allocator {
  DataPtr allocate(size_t size) const override {
    ...
    if (size != 0) {
      caching_allocator.malloc(&r, device, size, cuda::getCurrentCUDAStream(device), false);
    }
    return {r, r, &raw_delete, Device(DeviceType::CUDA, device)};
  }

  DataPtr FNallocate(size_t size) const override {
    int device;
    C10_CUDA_CHECK(cudaGetDevice(&device));
    void* r = nullptr;
    if (forceUncachedAllocator()) {
    ...
    if (size != 0) {
      caching_allocator.malloc(&r, device, size, cuda::getCurrentCUDAStream(device), true);
    }
    return {r, r, &raw_delete, Device(DeviceType::CUDA, device)};
  }
...
void* raw_alloc(size_t nbytes) {
  ...
  C10_CUDA_CHECK(cudaGetDevice(&device));
  void* r = nullptr;
  caching_allocator.malloc(&r, device, nbytes, cuda::getCurrentCUDAStream(device), false);
  return r;
}
...
void* raw_alloc_with_stream(size_t nbytes, cudaStream_t stream) {
  ...
  C10_CUDA_CHECK(cudaGetDevice(&device));
  void* r = nullptr;
  caching_allocator.malloc(&r, device, nbytes, stream, false);
  return r;
}
