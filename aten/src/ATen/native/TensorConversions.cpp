...
static inline Tensor to_impl(const Tensor& self, const TensorOptions& options, bool non_blocking, bool copy) {
  ...
  bool pin_out = (non_blocking && self.is_cuda() && options.device().is_cpu() && (options.layout() == c10::kStrided));
...
static inline Tensor FN_to_impl(const Tensor& self, const TensorOptions& options, bool non_blocking, bool is_csr, bool copy) {
  auto memory_format = options.memory_format_opt().value_or(MemoryFormat::Preserve);
...
*/
  ...
  r.FN_copy_(self, non_blocking, is_csr);
  return r;
}
...
Tensor FN_to(
  ...
  auto specified_options = self.options().merge_in(options);
  return FN_to_impl(self, specified_options, non_blocking, is_csr, copy);
}
...
Tensor FN_to(const Tensor& self, Device device, ScalarType dtype, bool non_blocking, bool is_csr, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format) {
  device = ensure_has_index(device);
  return FN_to_impl(
      self,
      self.options().device(device).dtype(dtype).memory_format(optional_memory_format),
...
Tensor FN_to(const Tensor& self, ScalarType dtype, bool non_blocking, bool is_csr, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format) {
  return FN_to_impl(
      self, self.options().dtype(dtype).memory_format(optional_memory_format), non_blocking, is_csr, copy);
}
...
Tensor FN_to(const Tensor& self, const Tensor& other, bool non_blocking, bool is_csr, bool copy, c10::optional<c10::MemoryFormat> optional_memory_format) {
  auto options = other.options();
  return FN_to_impl(self, options.memory_format(optional_memory_format), non_blocking, is_csr, copy);
}
