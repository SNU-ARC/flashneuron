...
static Tensor & FN_copy_impl(Tensor & self, const Tensor & src, bool non_blocking, bool is_csr) {
  // TODO: this should be handled during dispatch, but that's missing...
  TORCH_CHECK(self.defined(), "self is undefined");
  TORCH_CHECK(src.defined(), "src is undefined");
  ...
  FN_copy_stub(device_type, iter, non_blocking, self.unsafeGetIntrusivePtr()->tensor_id, is_csr);
  return self;
}


Tensor& FN_copy_(Tensor& self, const Tensor& src, bool non_blocking, bool is_csr) {
  auto maybe_outnames = namedinference::compute_broadcast_outnames(self, src);
  {
    NoNamesGuard guard;
    FN_copy_impl(self, src, non_blocking, is_csr);
  }
  namedinference::propagate_names_if_nonempty(self, maybe_outnames);
  return self;
}
